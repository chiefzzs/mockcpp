...
#if 0
static void clear_allocate_info(void* pointer)
{
    if (pointer == NULL)
        return;
    new_ptr_list_t* ptr =
    ...
    //free(ptr); // Do not free the real pointer, maybe it's still usefull, only free the allocate information.
    return;
}
...
void clear_all_allocate_info()
{
    ...
    while (ptr != &new_ptr_list)
    {
        ...
        clear_allocate_info((void*)pointer);
        ptr = ptr->next;
    }
}
#endif
